...
ABSL_FLAG(bool, fingerprintProvided, false,
          "True if fingerprint are provided in CSV file (for " FLAG_NAME(CSV) " preprocessing type only). "
                                                                              "Otherwise fingerprint based on baseLibrary"
                                                                              " will be used");

ABSL_FLAG(std::string, fingerprintType, "",
          "Type of fingerprint to be generated: " FLAG_NAME(Indigo) " or " FLAG_NAME(RDKit)
                  ". Specify " FLAG_NAME(Custom) " if fingerprint is provided");

ABSL_FLAG(uint64_t, fromFileWorkers, 1,
          "Number of workers to process queries from queriesFile if FromFile mode specified");

ABSL_FLAG(std::string, baseLibrary, "",
          "Library to use as basement for Qtr search. Possible options: Indigo, RDKit");
