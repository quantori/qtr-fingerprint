...
class BallTreeFinder(DriveFinder, ABC):
    ...
    def __init__(self, df: pd.DataFrame, directory: PathType, unique_id: Optional[str], *args, **kwargs) -> None:
        ball_tree = BallTree(df.values, leaf_size=2, metric='russellrao')
        self._pack((df.index, ball_tree), self.index_file)

    def find_all(self, fingerprint: pd.Series) -> Iterable[str]:
        df_index, ball_tree = self._unpack(self.index_file)
