...
class SplitterTreeFinder(DriveFinder):
    ...
    class Node:
        def __init__(self, base: SplitterTreeFinder, col_id: int, left: Optional[SplitterTreeFinder.Node],
                     right: Optional[SplitterTreeFinder.Node], df: pd.DataFrame,
    ...
    def __build_tree(self, df: pd.DataFrame, used_cols: List[int], depth: int,
                     node_path: str, *args, **kwargs) -> SplitterTreeFinder.Node:
        if len(df) < self.bucket_size or depth == 0:
            if len(df) == 0:
                Logger.log("WARNING: empty bucket")
            return self.Node(base=self, col_id=self.leaf_id, left=None, right=None, df=df, tree_path=node_path)
        ...
        return self.Node(self, split_id, left, right, df, node_path, *args, **kwargs)
